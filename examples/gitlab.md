  ...
  script:
   - |
    # ARGS:
            ...
            # CI ENGINE SPECIFIC:
            eval SOOS_CHECKOUT_DIR="$CI_PROJECT_DIR"
            ls -al $SOOS_CHECKOUT_DIR
            SOOS_COMMIT_HASH="${CI_COMMIT_SHA}"
            SOOS_BRANCH_NAME="${CI_COMMIT_BRANCH}"
            SOOS_BRANCH_URI="" # ENTER BRANCH URI HERE IF KNOWN
            SOOS_BUILD_VERSION="" # ENTER BUILD VERSION HERE IF KNOWN
            SOOS_BUILD_URI="${CI_JOB_URL}"
            SOOS_OPERATING_ENVIRONMENT="${CI_RUNNER_EXECUTABLE_ARCH}"
            SOOS_INTEGRATION_NAME="Gitlab"
